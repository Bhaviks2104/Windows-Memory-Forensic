#Name, Parent, Instance, Path
from parent_child_process import windows_processes, profiles, normalize_path
import re
...
def is_suspicious_name(process_name):
    ...
    if not process_name:  # Handle empty process names
        return False, []  # Not suspicious, no reasons

    if process_name.lower() not in (p.lower() for p in windows_processes):
        close_matches = get_close_matches(process_name, windows_processes, n=1, cutoff=0.9)  # Only pick 1 best match
        ...
        if isinstance(instances, dict):
            ...
            if profile_index <= instance_index_early: 
                ...
                if "minimum" in instances["Win2003SP2x86"] and actual_count < instances["Win2003SP2x86"]["minimum"]:
                        reasons.append({'reason': "Minimum {} instances required and found {}".format(instances["Win2003SP2x86"]["minimum"], actual_count),'category' : "Min_ins"})
                        return True, reasons
                if "maximum" in instances["Win2003SP2x86"] and actual_count > instances["Win2003SP2x86"]["maximum"]:
                        reasons.append({'reason': "Maximum {} instances required and found {}".format(instances["Win2003SP2x86"]["maximum"], actual_count),'category' : "Max_ins"})
                        return True, reasons
                ...
                if "minimum" in instances["VistaSP0x86"] and actual_count < instances["VistaSP0x86"]["minimum"]:
                        reasons.append({'reason': "Minimum {} instances required and found {}".format(instances["VistaSP0x86"]["minimum"], actual_count),'category' : "Min_ins"})
                        return True, reasons
                if "maximum" in instances["VistaSP0x86"] and actual_count > instances["VistaSP0x86"]["maximum"]:
                        reasons.append({'reason': "Maximum {} instances required and found {}".format(instances["VistaSP0x86"]["maximum"], actual_count),'category' : "Max_ins"})   
                        return True, reasons   
        else:
...
# Normalize the keys in the windows_processes dictionary to lowercase
...
def check_cmdline(cmdline):
    suspicious_patterns = [
        ...
        r'(?<!\w)bcdedit(?!\w)',
        r'(?<!\w)schtasks(?!\w)',
        r'(?<!\w)at(?!\w)',       # Now won't match "compatible"
        r'(?<!\w)sc(?!\w)',
        r'(?<!\w)net\s+user(?!\w)',
        r'(?<!\w)tasklist(?!\w)',
        r'(?<!\w)taskkill(?!\w)',
        r'(?<!\w)netsh(?!\w)',
        r'(?<!\w)mimikatz(?!\w)',
        r'(?<!\w)nc(?!\w)',       # Now won't match "DcomLaunch"
        r'(?<!\w)curl(?!\w)',
        r'(?<!\w)wget(?!\w)',
        r'(?<!\w)ftp(?!\w)',
        r'(?<!\w)tftp(?!\w)',
    ...
    # Whitelist of benign commands (lowercase)
    benign_commands = [
    ...
    reasons = []
    # Convert cmdline to lowercase for comparison
...
def check_path(process_name, dll_path, cmd_path, peb_path, vad_path):
    ...
    # Determine final path based on priority order
    ...
    # If process is svchost.exe, ensure it has the -k parameter
    ...
    # Check against expected path for known Windows processes
